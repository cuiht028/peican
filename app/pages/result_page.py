...
from app.services.meal_planner import replace_dish
from app.services.env_service import get_region

# 选中态配色
_COLOR_SELECTED = "#E8F5E9"  # 淡绿底 = 选中
_COLOR_NORMAL = "#F5F5F5"    # 默认卡片底
...
def result_page(page: ft.Page) -> ft.View:
    ...
    header = theme.card(
        ft.Column(
            [
                ...
                ft.Container(height=6),
                theme.text("点击菜品可选中（变绿），再点「更换配菜」替换选中菜品",
    ...
    actions = ft.Row(
        [
            theme.big_button("重新配餐", icon=ft.Icons.REFRESH, width=130,
                             on_click=lambda e: navigate(page, "/diner_select")),
            theme.big_button("更换配菜", icon=ft.Icons.SWAP_HORIZ, width=130,
                             bgcolor=theme.COLOR_PRIMARY_LIGHT,
                             on_click=_on_replace),
            theme.big_button("买菜清单", icon=ft.Icons.SHOPPING_CART, width=130,
                             bgcolor=theme.COLOR_ACCENT, on_click=_on_shopping),
        ],
        alignment=ft.MainAxisAlignment.CENTER,
        wrap=True,
    )
    save_btn = theme.big_button("保存今日配餐", icon=ft.Icons.SAVE, width=400,
